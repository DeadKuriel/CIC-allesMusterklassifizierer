...
class BooleanMapper(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        true_values: tuple = (True, 1, "true", "yes"),
        false_values: tuple = (False, 0, "false", "no"),
    ):
    ...
    def fit(self, X: Any, y: Any = None):
        ...
        if ambiguous or unknown:
            raise SchemaError(
                f"Valores booleanos ambiguos o desconocidos: {sorted(map(str, ambiguous | unknown))}"
            )
        return self
    ...
    def transform(self, X: Any):
        ...
        return frame.map(
            lambda x: np.nan if pd.isna(x) else (1.0 if x in self.true_values else 0.0)
        ).to_numpy()
        ...
        for col in pd.DataFrame(X).columns:
            ...
            out.extend(
                [
                    ...
                    s.dt.dayofweek,
                    s.dt.quarter,
                    np.sin(2 * np.pi * s.dt.month / 12),
                    np.cos(2 * np.pi * s.dt.month / 12),
                ]
            )
...
def build_preprocessor(cfg: ExperimentConfig) -> ColumnTransformer:
                ...
                Pipeline(
                    [
                        (
                            "imputer",
                            SimpleImputer(
                                strategy=p.numeric_imputation,
                                fill_value=p.numeric_constant,
                                add_indicator=p.add_missing_indicators,
                            ),
                        ),
                        ("scale", scale),
                    ]
    ...
    if nominal := by_type.get("nominal"):
        strategy = (
            "constant"
            if p.categorical_imputation in {"constant", "missing_category"}
            else "most_frequent"
        )
        transformers.append(
            (
                ...
                Pipeline(
                    [
                        (
                            ...
                            SimpleImputer(
                                strategy=strategy,
                                fill_value=p.categorical_constant,
                                add_indicator=p.add_missing_indicators,
                            ),
                        ),
                        (
                            ...
                            OneHotEncoder(
                                ...
                                min_frequency=p.min_category_frequency,
                            ),
                        ),
                    ]
                ),
            ...
            (
                ...
                Pipeline(
                    [
                        (
                            "mapper",
                            BooleanMapper(tuple(c.true_values), tuple(c.false_values)),
                        ),
                        ("imputer", SimpleImputer(strategy="most_frequent")),
                    ]
                ),
...
def build_pipeline(cfg: ExperimentConfig, estimator):
    ...
    if strategy != "none":
        ...
        if strategy == "smotenc" and "categorical_features" not in params:
            raise SchemaError(
                "SMOTENC requiere categorical_features explícito tras el preprocesamiento"
            )
