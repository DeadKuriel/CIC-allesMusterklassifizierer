...
def _pattern_groups(frame: pd.DataFrame, features: list[str]) -> list[tuple[Any, pd.DataFrame]]:
...
def audit_and_clean(dataset: TabularDataset, cfg: ExperimentConfig) -> AuditResult:
    ...
    if frame[cfg.target].isna().any() and cfg.audit.target_missing == "drop_rows":
        for idx in frame.loc[frame[cfg.target].isna(), "__original_index__"]:
            log.append({"original_index": idx, "action": "drop", "reason": "missing_target"})
    ...
    if cfg.audit.indiscernible_policy == "resolve_by_mapping":
        if not cfg.audit.resolution_mapping or not cfg.audit.resolution_mapping.is_file():
            raise AuditError("resolve_by_mapping requiere un archivo resolution_mapping existente")
        ...
        mapping = dict(zip(mapped["pattern_key"].astype(str), mapped["resolved_target"], strict=True))
    for key, group in _pattern_groups(frame, features):
        ...
        if len(labels) == 1 and len(group) > 1:
            representative = group.iloc[-1 if cfg.audit.redundant_policy == "keep_last" else 0]["__original_index__"]
            for _, row in group.iterrows():
                redundant_rows.append({"pattern_key": key_text, "original_index": row["__original_index__"],
                                       "representative_index": representative, "target": row[cfg.target],
                                       "action": cfg.audit.redundant_policy, "reason": "identical_X_and_y"})
            ...
            if cfg.audit.redundant_policy in {"keep_first", "keep_last"}:
                drop_positions.update(group.index[group["__original_index__"] != representative])
        elif len(labels) > 1:
            action = cfg.audit.indiscernible_policy
            for _, row in group.iterrows():
                conflict_rows.append({"pattern_key": key_text, "original_index": row["__original_index__"],
                                      "target": row[cfg.target], "action": action, "reason": "identical_X_different_y"})
            if action == "fail":
            ...
            if action == "remove_all":
                ...
                if key_text not in mapping:
                    raise AuditError(f"No hay resolución manual para pattern_key={key_text}")
                frame.loc[group.index, cfg.target] = mapping[key_text]
    for pos in sorted(drop_positions):
        log.append({"original_index": frame.loc[pos, "__original_index__"], "action": "drop", "reason": "pattern_policy"})
    ...
    for col in cfg.columns:
        if col.name not in frame:
            continue
        s = frame[col.name]
        columns.append({"column": col.name, "role": col.role, "semantic_type": col.semantic_type,
                        "physical_type": str(s.dtype), "missing_count": int(s.isna().sum()),
                        "missing_ratio": float(s.isna().mean()), "unique_count": int(s.nunique(dropna=True))})
    ...
    report = {"rows": len(frame), "columns": len(frame.columns), "column_summary": columns,
              "class_distribution": {str(k): int(v) for k, v in counts.items()}, "imbalance_ratio": ratio,
              "constant_columns": [c for c in frame if frame[c].nunique(dropna=False) == 1],
              "near_constant_columns": [c for c in frame if frame[c].value_counts(normalize=True, dropna=False).max() >= cfg.audit.near_constant_threshold],
              "high_cardinality_identifiers": [c.name for c in cfg.columns if c.role == "feature" and frame[c.name].nunique() / len(frame) > .9],
              "redundant_groups": len({r["pattern_key"] for r in redundant_rows}),
              "indiscernible_groups": len({r["pattern_key"] for r in conflict_rows}),
              "warnings": ([f"Desbalance {ratio:.2f}:1"] if ratio and ratio >= cfg.audit.imbalance_ratio_warning else [])}
    return AuditResult(cleaned, report, pd.DataFrame(log), pd.DataFrame(redundant_rows), pd.DataFrame(conflict_rows))


def write_audit(result: AuditResult, directory: Path) -> None:
    import json
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "data_audit.json").write_text(json.dumps(result.report, indent=2, ensure_ascii=False), encoding="utf-8")
