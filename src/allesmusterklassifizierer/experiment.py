...
def run(cfg: ExperimentConfig, *, compare=False) -> Path:
    ...
    rows=[]; fold_rows=[]; model_metrics={}; chosen=None; chosen_metric=-np.inf; best_payload=None
    for mc in [m for m in cfg.models if m.enabled]:
        ...
        try:
            if final_holdout:
                ...
                fit_time=time.perf_counter()-t0; p0=time.perf_counter(); evaluation_idx = test_idx
                ...
                if cfg.tuning.method != "none":
                    raise ValueError("K-fold externo con tuning requiere validación cruzada anidada")
                fold_true=[]; fold_pred=[]; fold_prob=[]; fold_indices=[]; fit_time=0.0; predict_time=0.0
                for sp in splits:
                    f0=time.perf_counter(); fold_pipeline, _, _ = _fit(cfg, mc, X.iloc[sp.train_idx], y.iloc[sp.train_idx], [])
                    fit_time += time.perf_counter()-f0; p0=time.perf_counter(); fold_prediction=fold_pipeline.predict(X.iloc[sp.test_idx])
                    predict_time += time.perf_counter()-p0; fold_probability=fold_pipeline.predict_proba(X.iloc[sp.test_idx]) if hasattr(fold_pipeline,"predict_proba") else None
                    fold_score=fold_probability[:,1] if fold_probability is not None and fold_probability.shape[1]==2 else None
                    fm=classification_metrics(y.iloc[sp.test_idx],fold_prediction,labels=list(fold_pipeline.classes_),scores=fold_score,probabilities=fold_probability,
                        average=cfg.evaluation.average,positive_class=cfg.evaluation.positive_class,zero_division=cfg.evaluation.zero_division)
                    fold_rows.append({"model":model_label,"algorithm":mc.name,"fold":sp.fold,"train_size":len(sp.train_idx),"test_size":len(sp.test_idx),
                                      **{k:v for k,v in fm.items() if k not in {"per_class","labels"}}})
                    fold_true.append(y.iloc[sp.test_idx].to_numpy()); fold_pred.append(fold_prediction); fold_indices.append(sp.test_idx)
                    if fold_probability is not None: fold_prob.append(fold_probability)
                f0=time.perf_counter(); pipeline, params, cv_results = _fit(cfg, mc, X, y, [])
                fit_time += time.perf_counter()-f0
                evaluation_idx=np.concatenate(fold_indices); eval_y=pd.Series(np.concatenate(fold_true)); pred=np.concatenate(fold_pred)
                probs=np.concatenate(fold_prob) if len(fold_prob)==len(splits) else None
            scores=pipeline.decision_function(eval_X) if final_holdout and hasattr(pipeline,"decision_function") else (probs[:,1] if probs is not None and probs.shape[1]==2 else None)
            labels=list(pipeline.classes_); metrics=classification_metrics(eval_y,pred,labels=labels,scores=scores,probabilities=probs,
                average=cfg.evaluation.average,positive_class=cfg.evaluation.positive_class,zero_division=cfg.evaluation.zero_division)
            score=float(metrics.get(cfg.evaluation.primary_metric,metrics["balanced_accuracy"])); model_metrics[model_label]=metrics
            rows.append({"model":model_label,"algorithm":mc.name,"selection_metric":score,"accuracy":metrics["accuracy"],
                         "balanced_accuracy":metrics["balanced_accuracy"],"precision":metrics["precision"],
                         "recall":metrics["recall"],"f1":metrics["f1"],"mcc":metrics["mcc"],
                         "cohen_kappa":metrics["cohen_kappa"],"roc_auc":metrics.get("roc_auc"),
                         "pr_auc":metrics.get("pr_auc"),"log_loss":metrics.get("log_loss"),
                         "fit_seconds":fit_time,"predict_seconds":predict_time,
                         "best_params":json.dumps(params),"status":"success","error":""})
            if score>chosen_metric: chosen_metric=score; chosen=model_label; best_payload=(pipeline,params,metrics,eval_y,pred,probs,evaluation_idx,cv_results)
    ...
    pd.DataFrame(rows).to_csv(run_dir/"cv_results.csv",index=False); pd.DataFrame(fold_rows or cv_results).to_csv(run_dir/"fold_metrics.csv",index=False)
    ...
    write_json(run_dir/"final_metrics.json",metrics); write_json(run_dir/"model_metrics.json",model_metrics)
    write_json(run_dir/"splits.json",[{"fold":s.fold,"train":s.train_idx.tolist(),"validation":s.validation_idx.tolist() if s.validation_idx is not None else None,"test":s.test_idx.tolist()} for s in splits])
    ...
    if cfg.persistence.report_html: html_report(run_dir,cfg.run_name,{"Resumen ejecutivo":manifest,"Calidad del dataset":audit.report,"Comparación":rows,"Métricas por fold":fold_rows,"Métricas por modelo":model_metrics,"Métricas del modelo seleccionado":metrics,"Reproducibilidad":{"seed":cfg.seed,"splits":"splits.json"}})
    return run_dir
