...
def load_model(path: str | Path) -> ModelArtifact:
    try:
        ...
        raise ArtifactError(f"No se pudo cargar el modelo: {exc}") from exc
...
def write_base(
    run_dir: Path, cfg, schema: dict[str, Any], manifest: dict[str, Any]
) -> None:
...
def html_report(run_dir: Path, title: str, sections: dict[str, Any]) -> None:
    ...
    blocks = [
        f"<h2>{html.escape(name)}</h2><pre>{html.escape(json.dumps(value, indent=2, ensure_ascii=False, default=str))}</pre>"
