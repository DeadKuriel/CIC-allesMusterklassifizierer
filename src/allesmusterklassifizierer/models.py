...
REGISTRY: dict[str, ModelSpec] = {
    "logistic_regression": ModelSpec(
        LogisticRegression, {"max_iter": 1000}, True, True, True, {"C": [0.1, 1, 10]}
    ),
    "knn": ModelSpec(
        KNeighborsClassifier, {}, True, False, True, {"n_neighbors": [1, 3, 5, 9]}
    ),
    "centroid": ModelSpec(NearestCentroid, {}, False, False, True, {}),
    "decision_tree": ModelSpec(
        DecisionTreeClassifier, {}, True, True, False, {"max_depth": [None, 3, 8]}
    ),
    ...
    "gradient_boosting": ModelSpec(
        GradientBoostingClassifier, {}, True, False, False, {"n_estimators": [50, 100]}
    ),
    "hist_gradient_boosting": ModelSpec(
        HistGradientBoostingClassifier, {}, True, True, False, {"max_iter": [100, 200]}
    ),
    "adaboost": ModelSpec(
        AdaBoostClassifier, {}, True, False, False, {"n_estimators": [50, 100]}
    ),
    "mlp": ModelSpec(
        MLPClassifier,
        {"max_iter": 500},
...
def normalize_distance(value: Any) -> tuple[str, float]:
    ...
    if name not in DISTANCE_ALIASES:
        raise ClassifierError(
            f"Distancia no soportada: {name}. Usa {sorted(DISTANCE_ALIASES)}"
        )
...
def create_model(name: str, params: dict[str, Any], seed: int) -> BaseEstimator:
    ...
    if name not in REGISTRY:
        raise ClassifierError(
            f"Modelo desconocido '{name}'. Disponibles: {sorted(REGISTRY)}"
        )
    spec = REGISTRY[name]
    ...
    if name == "knn" and "distance" in values:
        ...
        if metric == "minkowski":
            values["p"] = p
    if (
        "random_state" in spec.constructor().get_params()
        and "random_state" not in values
    ):
...
class AcademicCentroid(BaseEstimator, ClassifierMixin):
    def fit(self, X, y):
        ...
        self.centroids_ = np.stack(
            [np.mean(X[np.asarray(y) == c], axis=0) for c in self.classes_]
        )
        return self
    ...
    def predict(self, X):
        ...
        return self.classes_[
            np.argmin(
                ((np.asarray(X)[:, None] - self.centroids_) ** 2).sum(axis=2), axis=1
            )
        ]
