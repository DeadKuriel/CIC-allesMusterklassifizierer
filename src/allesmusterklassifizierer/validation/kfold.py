    ...
    def split(self, y: np.ndarray) -> Iterable[Split]:
        ...
        if self.n_splits > n:
            raise ValidationError(
                f"n_splits={self.n_splits} no puede ser mayor que n_samples={n}."
            )
        ...
        if not self.stratify:
            ...
            for fold_id in range(self.n_splits):
                ...
                train_idx = np.sort(
                    np.concatenate(
                        [folds[j] for j in range(self.n_splits) if j != fold_id]
                    )
                )
                if test_idx.size == 0 or train_idx.size == 0:
                    raise ValidationError(
                        "K-Fold produjo un fold vacío. Revisa n_splits."
                    )
        ...
        for fold_id in range(self.n_splits):
            ...
            train_idx = np.sort(
                np.concatenate(
                    [fold_arrays[j] for j in range(self.n_splits) if j != fold_id]
                )
            )
            if test_idx.size == 0 or train_idx.size == 0:
                raise ValidationError(
